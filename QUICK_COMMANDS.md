...
## 🚀 СПОСОБ 1: Одна команда (запуск с локального ПК)

```bash
# Из папки wam-stack на ЛОКАЛЬНОМ ПК
bash scripts/remote-deploy.sh

# С пересборкой Docker образа
bash scripts/remote-deploy.sh --build

# С перезагрузкой всех сервисов
bash scripts/remote-deploy.sh --reset
```

Этот скрипт автоматически:
1. Синхронизирует код на сервер
2. SSH на сервер и запускает deploy.sh
3. Устанавливает WAM_MODEL=unifolm

---

## 🚀 СПОСОБ 2: Полный цикл запуска вручную (3 терминала)
